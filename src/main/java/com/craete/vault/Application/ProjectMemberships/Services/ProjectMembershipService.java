...
public class ProjectMembershipService implements IProjectMembershipService {
    ...
    @Override
    @Transactional
    public ProjectMembershipStorageModel createProjectMembership(ProjectMembershipCreateModel ProjectMembershipCreateModel) {
        if (ProjectMembershipCreateModel == null) throw new IllegalArgumentException("Project membership can't be null.");
    ...
    public ProjectMembershipStorageModel getProjectMembershipById(UUID id) {
        ...
        return modelMapper.map(
            projectMembershipRepository.findById(id)
                .orElseThrow(() -> new ProjectMembershipNotFoundException(String.format("Membership with id %s not found", id))),
    ...
    public List<ProjectMembershipStorageModel> getProjectMembershipsById(List<UUID> ids) {
        ...
        if (savedMemberships.isEmpty()) throw new ProjectMembershipNotFoundException("Memberships not found.");
    ...
    @Override
    @Transactional
    public ProjectMembershipStorageModel patchProjectMembership(ProjectMembershipPatchModel ProjectMembershipPatchModel) {
        if (ProjectMembershipPatchModel == null) throw new IllegalArgumentException("Project membership can't be null");
        ProjectMembership existingMembership = projectMembershipRepository.findById(ProjectMembershipPatchModel.getId())
            .orElseThrow(
                () -> new ProjectMembershipNotFoundException(
                    String.format("Project membership with id %s not found", ProjectMembershipPatchModel.getId())
                )
            );
        modelMapper.map(ProjectMembershipPatchModel, existingMembership);
        return modelMapper.map(projectMembershipRepository.save(existingMembership), ProjectMembershipStorageModel.class);
    }
    ...
    public void deleteProjectMembershipById(UUID id) {
        ...
        projectMembershipRepository.findById(id)
            .orElseThrow(
                () -> new ProjectMembershipNotFoundException(String.format("Membership with id %s not found", id))
            );
    ...
    public void deleteProjectMembershipByUserId(Long UserId) {
        if (UserId == null) throw new IllegalArgumentException("id can't be null");
        if (projectMembershipRepository.findByUserId(UserId).isEmpty()) throw new ProjectMembershipNotFoundException(String.format("Memberships of user with id %s not found", UserId));
        projectMembershipRepository.deleteAllByUserId(UserId);
    }
    ...
    public void deleteProjectMembershipByProjectId(UUID ProjectId) {
        ...
        if (projectMembershipRepository.findByProjectId(ProjectId).isEmpty()) throw new ProjectMembershipNotFoundException(String.format("Memberships of project with id %s not found", ProjectId));
